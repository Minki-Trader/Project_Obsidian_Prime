...
RUN_SPECS = [
    {
        "stage_id": "41I",
        "folder_name": "41I_34d_bridge_contract0675_0001",
        "experiment_id": "exp_41i_34d_bridge_contract0675_v1",
        "min_margin": 0.06750,
        "label": "34d_contract_margin0675_bridge",
    },
    {
        "stage_id": "41J",
        "folder_name": "41J_34d_bridge_contract0600_0001",
        "experiment_id": "exp_41j_34d_bridge_contract0600_v1",
        "min_margin": 0.06000,
        "label": "34d_contract_margin0600_bridge",
    },
]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create Stage 41 bridge confirmation run directories.")
    parser.add_argument(
        "--only-stage-ids",
        help="Optional comma-separated subset such as 41I,41J. When omitted, all specs are considered.",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
...
def build_status_history() -> list[StatusEvent]:
    now = utc_now_iso()
    return [
        StatusEvent(status="draft", changed_at_utc=now, reason="bundle_initialized_from_stage39_34d_bridge_source"),
        StatusEvent(status="ready", changed_at_utc=now, reason="stage41_margin_bridge_confirmation_prepared"),
    ]
...
def main() -> int:
    ...
    for spec in RUN_SPECS:
        ...
        bundle.runtime_snapshot.extra["stage41_margin_bridge_label"] = spec["label"]
        bundle.runtime_snapshot.extra["stage41_margin_bridge_mode"] = "extended_bridge_contract_confirmation"
        bundle.runtime_snapshot.extra["stage41_margin_bridge_min_margin"] = float(spec["min_margin"])
