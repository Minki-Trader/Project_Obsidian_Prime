...
RUNS = {
    "27A": {"folder": "27A_26a_volref_0001", "label": "volatility observe-only inherited baseline"},
    "27B": {"folder": "27B_26a_vh080_0001", "label": "high-volatility taper only"},
    "27C": {"folder": "27C_26a_vl105h080_0001", "label": "two-sided mild volatility bucket overlay"},
    "27D": {"folder": "27D_26a_vl110h085_0001", "label": "two-sided stronger volatility bucket overlay"},
}

OUTPUT_JSON = REVIEW_DIR / "stage27_vol_adaptive_wave1_20260409.json"
OUTPUT_MD = REVIEW_DIR / "stage27_vol_adaptive_wave1_20260409.md"
...
def pick_best_challenger(results: dict[str, Any]) -> str:
    candidates = [code_name for code_name in results["runs"] if code_name != "27A"]
    return max(
        candidates,
...
def build_results() -> dict[str, Any]:
    ...
    regular_incumbent = runs[best_challenger_code]["folder"] if decision == "replace_incumbent" else runs["27A"]["folder"]
    regular_shadow = runs["27A"]["folder"] if decision == "replace_incumbent" else runs[best_challenger_code]["folder"]
    return {
...
def write_review_markdown(results: dict[str, Any]) -> None:
    ...
    purpose_text = "reuse the EA's ATR14/ATR50 regime ratio as a non-blocking volatility-aware risk overlay on the 26A regular baseline"
    lines = [
        "# Stage 27 Volatility-Adaptive Overlay Wave 1",
        "",
        f"- reviewed_on: {code(results['reviewed_on'])}",
        f"- purpose: {code(purpose_text)}",
        ...
        *render_markdown_table(
        ...
        "## Headline",
        "",
        f"- baseline `27A`: {code({'hist': baseline['hist_2024']['headline']['return_pct'], 'val': baseline['validation']['headline']['return_pct'], 'test': baseline['test']['headline']['return_pct'], 'test_dd': baseline['test']['headline']['max_dd_pct'], 'test_pf': baseline['test']['headline']['profit_factor']})}",
        ...
        f"- this wave should be read as a `market-state overlay` experiment, so `overlay_rate` and containment matter alongside raw headline return.",
        "",
        "## Execution",
        "",
        f"- baseline execution read: {code({'skip_rate': baseline['test']['execution']['skip_rate'], 'external_mismatch_count': baseline['test']['execution']['external_mismatch_count'], 'mean_overlay_rate': baseline['test']['governance']['mean_overlay_rate'], 'latest_state': baseline['test']['governance']['latest_state']})}",
        f"- challenger execution read: {code({'skip_rate': challenger['test']['execution']['skip_rate'], 'external_mismatch_count': challenger['test']['execution']['external_mismatch_count'], 'mean_overlay_rate': challenger['test']['governance']['mean_overlay_rate'], 'latest_state': challenger['test']['governance']['latest_state']})}",
        "",
        "## Decision",
        "",
        ...
        f"- why: {code('promote only if the volatility bucket improves the regular OOS read or clearly improves containment without obvious non-OOS damage')}",
    ]
...
def write_selection_markdown(results: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- gate_3: {code('prefer volatility overlays that improve the risk read without turning into a hidden leverage boost story')}",
        "",
        "## Scoreboards",
        "",
        f"- regular_risk_execution: {code('active for this wave')}",
        f"- structural_scout: {code('not separated in wave 1; this is already a live-like risk overlay test')}",
        "",
        "## Headline",
        "",
        f"- baseline `27A`: {code({'hist': baseline['hist_2024']['headline']['return_pct'], 'val': baseline['validation']['headline']['return_pct'], 'test': baseline['test']['headline']['return_pct'], 'test_dd': baseline['test']['headline']['max_dd_pct'], 'test_pf': baseline['test']['headline']['profit_factor']})}",
        ...
        "## Execution",
        "",
        f"- baseline execution: {code({'skip_rate': baseline['test']['execution']['skip_rate'], 'external_mismatch_count': baseline['test']['execution']['external_mismatch_count'], 'mean_overlay_rate': baseline['test']['governance']['mean_overlay_rate'], 'latest_state': baseline['test']['governance']['latest_state']})}",
        f"- challenger execution: {code({'skip_rate': challenger['test']['execution']['skip_rate'], 'external_mismatch_count': challenger['test']['execution']['external_mismatch_count'], 'mean_overlay_rate': challenger['test']['governance']['mean_overlay_rate'], 'latest_state': challenger['test']['governance']['latest_state']})}",
        "",
        "## Decision",
        "",
        f"- decision: {code(results['decision'])}",
        ...
        "## Follow-Up Bias",
        "",
        f"- follow_up_1: {code('keep the volatility overlay line only if it improves the risk read without obvious non-OOS drag')}",
        f"- follow_up_2: {code('if a high-vol taper helps but two-sided buckets overfit, keep the high-vol containment interpretation and avoid low-vol leverage chasing')}",
        "",
        "## Report Refs",
        "",
        f"- {code('03_reviews/stage27_vol_adaptive_wave1_20260409.md')}",
    ]
...
def write_review_index(results: dict[str, Any]) -> None:
    lines = [
        "# Stage 27 Review Index",
        "",
        "## Reading Order",
        "",
        f"1. {code('00_spec/stage_brief.md')}",
        f"2. {code('03_reviews/stage27_vol_adaptive_wave1_20260409.md')}",
        ...
        f"- {code('Stage 26 governance adaptive overlay remains supporting context; do not reopen governance threshold sweeps inside Stage 27.')}",
    ]
