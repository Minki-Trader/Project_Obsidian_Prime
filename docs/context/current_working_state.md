...
- Stage 41 targeted feature snapshot audit status: `wave1_completed`
- Stage 41 targeted feature snapshot audit decision: `localized_python_to_mt5_feature_parity_recovered_after_contract_matching_runtime_patch`
- Stage 41 targeted feature snapshot audit read: `the localized Stage 40 cluster windows closed to float-noise parity after replacing the MT5 feature-path ATR and Stochastic calculations with custom contract-matching implementations; post-patch att_0005 reduced row max abs diff from about 29.672052 worst-case to about 0.000006, with all 14 audited rows inside 1e-4 tolerance`
- Stage 41 shared feature-path attribution read: `a broader latest-window feature-snapshot sample now points to a shared thresholding effect rather than a 34D-only rule effect: across 121 sampled built-in-versus-contract rows on the simpler 34B bundle, atr_14, atr_50, atr_14_over_atr_50, and secondarily stoch_kd_diff dominated the drift surface, the contract-aligned path shifted probability mass toward p_flat, and 26 of 32 sampled decision flips moved from built-in directional entries to contract NO_TRADE`
- Stage 41 threshold sensitivity read: `the same 121-row sample now narrows that story to a fixed-base decision-boundary tax: 25 of the 26 sampled directional-to-NO_TRADE flips are max_probability_margin failures, only one sampled row is a pure threshold-cross, and none of the 32 sampled decision-diff rows activated a relevant contextual soft suppressor, which points the next attribution step toward controlled min_margin replay around 0.0675 rather than more sidecar hunting`
- Stage 41 margin replay read: `a fresh latest-window contract-aligned min_margin replay on 34B now shows that 0.0600 is the only tested value in the coarse 0.0600 / 0.0625 / 0.0650 / 0.0675 / 0.0700 sweep that materially improves the KPI surface; versus the 0.0675 contract baseline it recovered net by 10.39, return_pct by 2.078, trade_count by 15, profit_factor by 0.0847, and max_dd_pct by 0.9851, and the same 0.0600 replay matched exactly on the operating 34D bundle`
- Stage 41 margin fine replay read: `a focused 34D-only follow-up on 0.05875 / 0.06000 / 0.06125 now keeps 0.06000 as the local best tested operating point; 0.05875 added trade count and slightly higher profit factor but gave back net by 0.84 and raised max_dd_pct by 1.1362, while 0.06125 weakened net by 5.76 and profit_factor by 0.0239 versus the 0.06000 centerpoint`
- Stage 41 margin bridge confirmation read: `a broader 34D contract-aligned extended-bridge follow-up on 2024.01.01 -> 2026.04.13 now also keeps 0.06000 above the 0.0675 contract baseline, recovering net by 834.45, return_pct by 166.890, trade_count by 200, profit_factor by 0.0113, max_dd_pct by 0.4561, and ulcer_index by 1.6069 while leaving external mismatch pressure unchanged`
- Stage 41 latest live read: `a post-patch latest-window live-like run on 2026-03-01 through 2026-04-13 (att_0006) produced net -100.75, return_pct -20.150, trade_count 91, and profit_factor about 0.5511; this is the first saved trading-enabled run for that latest window, so it is a valid post-patch KPI read but not yet an exact pre-vs-post A/B delta`
- Stage 41 latest live A/B read: the corrected latest-window A/B replay now shows a shared feature-path effect instead of a `34D`-only effect: genuine built-in runs (`39A att_0007` for `34D` and `39B att_0004` for `34B`) match each other exactly, genuine contract-aligned runs (`39A att_0008` for `34D` and `39B att_0003` for `34B`) also match each other exactly, and the contract-aligned lane is weaker than the built-in lane by `net -16.28`, `return_pct -3.256`, `trade_count -8`, `profit_factor -0.0862`, and `max_dd_pct +4.0484` on both families
- Stage 41 companion 34B A/B read: the earlier `39B att_0002` artifact was mislabeled: `tester_attempt_summary.json` recorded `use_contract_aligned_feature_indicators=false` while the saved `tester_run.ini` actually set `InpUseContractAlignedFeatureIndicators=true`; a corrective rerun as `39B att_0004` restored the genuine built-in lane and retired the earlier false `34D`-specific interpretation
