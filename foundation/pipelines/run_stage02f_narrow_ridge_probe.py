...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage 02F narrow ridge probe inside the current bracket.")
    parser.add_argument(
        "--source-run-dir",
    ...
    parser.add_argument(
        "--short-thresholds",
        default="0.540,0.545,0.550,0.555,0.560,0.565,0.570,0.575,0.580,0.585,0.590,0.595,0.600",
        help="Comma-separated short-threshold values for the narrow probe.",
    )
    parser.add_argument(
        "--long-thresholds",
        default="0.380,0.385,0.390,0.395,0.400",
        help="Comma-separated long-threshold values for the narrow probe.",
    )
    return parser
...
def update_selection_status(selection_status_path: Path, blended_leader: dict, ridge_note: str) -> None:
    lines = [
        ...
        (
            f"- latest narrow-ridge blended leader: "
            f"`(short_threshold={blended_leader['short_threshold']:.3f}, long_threshold={blended_leader['long_threshold']:.3f})`"
        ),
        f"- narrow-ridge leader valid compounded return: `{blended_leader['valid_compounded_return']:.4f}`",
        f"- narrow-ridge leader test compounded return: `{blended_leader['test_compounded_return']:.4f}`",
        f"- narrow-ridge read: `{ridge_note}`",
        "- final threshold freeze: `still deferred`",
        "- next phase: `continue tiny probes around the ridge only if the user still wants exploration`",
    ]
...
def main() -> int:
    ...
    run_dir = archived_dir / "02F_run_0001_narrow_ridge_probe"
    ...
    probe_df["direction_imbalance"] = (
        (probe_df["valid_long_trade_count"] - probe_df["valid_short_trade_count"]).abs()
        + (probe_df["test_long_trade_count"] - probe_df["test_short_trade_count"]).abs()
    )

    probe_df.to_csv(run_dir / "narrow_ridge_probe_results.csv", index=False)
    ...
    blended_sorted.to_csv(run_dir / "narrow_ridge_probe_blended_sorted.csv", index=False)

    stable_positive = blended_sorted[blended_sorted["both_positive"]].copy().reset_index(drop=True)
    stable_positive.to_csv(run_dir / "narrow_ridge_probe_stable_positive.csv", index=False)

    test_sorted = probe_df.sort_values(
        by=["test_compounded_return", "avg_compounded_return", "valid_compounded_return"],
        ascending=[False, False, False],
    ).reset_index(drop=True)
    test_sorted.to_csv(run_dir / "narrow_ridge_probe_test_sorted.csv", index=False)

    valid_sorted = probe_df.sort_values(
        by=["valid_compounded_return", "avg_compounded_return", "test_compounded_return"],
        ascending=[False, False, False],
    ).reset_index(drop=True)
    valid_sorted.to_csv(run_dir / "narrow_ridge_probe_valid_sorted.csv", index=False)

    blended_leader = stable_positive.iloc[0].to_dict()
    top_test = test_sorted.iloc[0].to_dict()
    top_valid = valid_sorted.iloc[0].to_dict()

    summary_payload = {
        "generated_at_utc": utc_now_iso(),
        "phase": "02F_narrow_ridge_probe",
        ...
        "top_valid_pair": top_valid,
        "verdict": "the useful ridge remains narrow around short thresholds in the lower 0.54-0.59 band and long thresholds at the 0.40 edge; keep exploration small",
    }
    write_json(run_dir / "summary.json", summary_payload)
    write_json(review_dir / "02F_narrow_ridge_probe_review.json", summary_payload)
    ...
    review_lines = [
        ...
        f"- short thresholds: `{', '.join(f'{value:.3f}' for value in short_values)}`",
        ...
        "",
        (
            f"- best both-positive blended pair: `(Ts={blended_leader['short_threshold']:.3f}, "
        ...
        "",
        (
            f"- top valid pair in the narrow ridge: `(Ts={top_valid['short_threshold']:.3f}, "
            f"Tl={top_valid['long_threshold']:.3f})`, valid_comp=`{top_valid['valid_compounded_return']:.4f}`, "
            f"test_comp=`{top_valid['test_compounded_return']:.4f}`"
        ),
        (
            f"- top test pair in the narrow ridge: `(Ts={top_test['short_threshold']:.3f}, "
            f"Tl={top_test['long_threshold']:.3f})`, valid_comp=`{top_test['valid_compounded_return']:.4f}`, "
    ...
    for row in stable_positive.head(12).itertuples(index=False):
        review_lines.append(
            f"- `(Ts={row.short_threshold:.3f}, Tl={row.long_threshold:.3f})`: "
    ...
    review_lines.extend(
        [
            ...
            (
                "- read: inside the current bracket, the blended ridge still sits near the lower end of the short "
                "threshold range and at the upper edge of the long-threshold range. That means the useful pocket "
                "looks narrower than the full `0.55-0.70 / 0.35-0.40` bracket."
            ),
        ]
    )
    write_text(review_dir / "02F_narrow_ridge_probe_review.md", "\n".join(review_lines) + "\n")
    ...
    note_payload = {
        ...
        "verdict": "exploration continues inside the narrow ridge; no freeze",
    }
    write_json(selected_dir / "02F_narrow_ridge_probe.json", note_payload)

    note_lines = [
        "# 02F Narrow Ridge Probe",
        "",
        "- verdict: `exploration continues inside the narrow ridge; no freeze`",
        (
    ...
    write_text(selected_dir / "02F_narrow_ridge_probe.md", "\n".join(note_lines) + "\n")

    update_review_index(review_dir / "review_index.md")
    update_selection_status(
        selection_status_path=selected_dir / "selection_status.md",
        blended_leader=blended_leader,
        ridge_note="the ridge looks narrower and still hugs long_threshold=0.400 while short_threshold concentrates in the lower 0.54-0.59 band",
    )
