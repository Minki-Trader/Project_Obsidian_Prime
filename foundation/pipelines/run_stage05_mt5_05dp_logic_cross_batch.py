...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run non-additive logic exploration on top of promoted 05DP.")
    ...
    parser.add_argument(
        "--skip-holdout",
        action="store_true",
        help="Run validation sweep only and do not open holdout for the top-two new logic-cross candidates.",
    )
    return parser
...
def build_specs() -> list[LogicCrossSpec]:
    return [
        LogicCrossSpec(
            stage_id="05EK",
            run_name="05EK_05dp_diff_only_hold5_0001",
            experiment_id="exp_05ek_05dp_diff_only_hold5_v1",
            review_basename="05EK_mt5_logic_cross_review",
            mix_label="diff_only_hold5",
        ...
        LogicCrossSpec(
            stage_id="05EL",
            run_name="05EL_05dp_combo_loose_hold5_0001",
            experiment_id="exp_05el_05dp_combo_loose_hold5_v1",
            review_basename="05EL_mt5_logic_cross_review",
            mix_label="combo_loose_hold5",
            short_threshold=1.0 / 3.0,
            long_threshold=1.0 / 3.0,
            min_margin=0.0500,
        ...
        LogicCrossSpec(
            ...
            review_basename="05EM_mt5_logic_cross_review",
            mix_label="short_bias_margin_hold5",
        ...
        LogicCrossSpec(
            stage_id="05EN",
            run_name="05EN_05dp_short_bias_diff_hold5_0001",
            experiment_id="exp_05en_05dp_short_bias_diff_hold5_v1",
            review_basename="05EN_mt5_logic_cross_review",
            mix_label="short_bias_diff_hold5",
            short_threshold=0.3000,
            long_threshold=0.4500,
            min_probability_diff=0.0775,
        ),
        LogicCrossSpec(
            stage_id="05EO",
            run_name="05EO_05dp_short_bias_combo_hold5_0001",
            experiment_id="exp_05eo_05dp_short_bias_combo_hold5_v1",
            review_basename="05EO_mt5_logic_cross_review",
            mix_label="short_bias_combo_hold5",
            short_threshold=0.3000,
            long_threshold=0.4500,
            min_margin=0.0500,
            min_probability_diff=0.0700,
        ...
        LogicCrossSpec(
            stage_id="05EP",
            run_name="05EP_05dp_balanced_tight_margin_hold5_0001",
            experiment_id="exp_05ep_05dp_balanced_tight_margin_hold5_v1",
            review_basename="05EP_mt5_logic_cross_review",
            mix_label="balanced_tight_margin_hold5",
...
def build_rule_stack(spec: LogicCrossSpec) -> dict[str, Any]:
...
def ensure_bundle(spec: LogicCrossSpec, run_dir: Path, source_run_dir: Path, rebuild_bundle: bool) -> Path:
    ...
    write_json(
        manifest_path,
        {
            ...
            "candidate_type": "05dp_logic_cross",
...
def extract_payload(view: BundleView, spec: LogicCrossSpec | None, label: str) -> dict[str, Any]:
...
def build_review_payload(
    *,
    ...
    payload: dict[str, Any] = {
        "generated_at_utc": utc_now_iso(),
        "phase": "05EQ_mt5_05dp_logic_cross_batch",
    ...
    if ranked_holdout is not None:
        ...
        if holdout_winner not in REFERENCE_RUN_NAMES and validation_winner == holdout_winner:
            payload["verdict"] = {
                ...
                "reason": "logic-cross candidate led both validation and holdout versus promoted 05DP",
            }
        elif holdout_winner not in REFERENCE_RUN_NAMES:
            payload["verdict"] = {
                "status": "mixed_holdout_win_keep_incumbent",
                "selected_run_name": incumbent_name,
                "reason": "logic-cross candidate improved holdout but did not lead validation, so 05DP remains promoted",
            }
        else:
            payload["verdict"] = {
...
def build_review_markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["ranked_validation_runs"]:
        ...
        lines.append(
            ...
            f"mix=`{row['mix_label']}`, short_thr={short_repr}, long_thr={long_repr}, margin={margin_repr}, diff={diff_repr}, "
...
def update_review_index(review_dir: Path) -> None:
    ...
    entry = "- `05EQ`: see `05EQ_mt5_05dp_logic_cross_review.md`"
...
def main() -> int:
    ...
    review_json_path = review_dir / "05EQ_mt5_05dp_logic_cross_review.json"
    review_md_path = review_dir / "05EQ_mt5_05dp_logic_cross_review.md"
