...
REFERENCE_RUN_NAMES = [
    ...
    "05CC_trend_proxy_persistence_downside_riskoff_0001",
    "05CA_trend_proxy_persistence_riskoff_0001",
    "05BF_trend_proxy_persistence_volatility_0001",
]
REFERENCE_LABELS = {
    "05DJ_05bf_margin_hold5_0001": "promoted_05dj",
    "05CC_trend_proxy_persistence_downside_riskoff_0001": "downside_riskoff_reference",
    "05CA_trend_proxy_persistence_riskoff_0001": "riskoff_reference",
    "05BF_trend_proxy_persistence_volatility_0001": "persistence_vol_reference",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run downside-feature x exit-discipline cross candidates for Stage 05.")
    ...
    parser.add_argument("--min-margin", type=float, default=0.07, help="Margin filter value copied from the promoted logic.")
...
def build_specs() -> list[CrossSpec]:
    return [
        CrossSpec(
            stage_id="05DL",
            run_name="05DL_05cc_margin_hold5_0001",
            experiment_id="exp_05dl_05cc_margin_hold5_v1",
            review_basename="05DL_mt5_model_family_trial_review",
            variant_label="05cc_hold5",
            component_groups=[
        ...
        CrossSpec(
            stage_id="05DM",
            run_name="05DM_05cc_margin_hold4_0001",
            experiment_id="exp_05dm_05cc_margin_hold4_v1",
            review_basename="05DM_mt5_model_family_trial_review",
            variant_label="05cc_hold4",
            component_groups=[
                "trend_proxy_persistence",
                "trend_proxy_downside_pressure",
                "trend_proxy_risk_off_confirmation",
        ...
        CrossSpec(
            stage_id="05DN",
            run_name="05DN_05ca_margin_hold5_0001",
            experiment_id="exp_05dn_05ca_margin_hold5_v1",
            review_basename="05DN_mt5_model_family_trial_review",
            variant_label="05ca_hold5",
            component_groups=[
                "trend_proxy_persistence",
                "trend_proxy_risk_off_confirmation",
            ],
...
def run_trial(spec: CrossSpec, args: argparse.Namespace, *, run_holdout: bool) -> None:
    cmd = [
        sys.executable,
...
def extract_payload(view: BundleView, spec: CrossSpec | None, label: str) -> dict[str, Any]:
...
def build_review_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "phase": "05DO_mt5_downside_exit_cross_batch",
    ...
    if ranked_holdout is not None:
        ...
        if holdout_winner not in REFERENCE_RUN_NAMES and validation_winner == holdout_winner:
            payload["verdict"] = {
                ...
                "reason": "downside-feature and hold-discipline cross candidate led both validation and holdout versus the promoted incumbent",
            }
            ...
            payload["verdict"] = {
                ...
                "selected_run_name": incumbent_name,
                "reason": "no downside-feature x hold-discipline cross candidate cleared the promoted incumbent on holdout",
            }
    else:
        payload["verdict"] = {
...
def build_review_markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["ranked_validation_runs"]:
        ...
        lines.append(
            ...
            f"variant=`{row['variant_label']}`, groups=`{groups}`, hold={hold_repr}, "
...
def update_review_index(review_dir: Path) -> None:
    ...
    entry = "- `05DO`: see `05DO_mt5_downside_exit_cross_review.md`"
...
def main() -> int:
    ...
    review_json_path = review_dir / "05DO_mt5_downside_exit_cross_review.json"
    review_md_path = review_dir / "05DO_mt5_downside_exit_cross_review.md"
