...
class StackSpec:
    ...
    review_basename: str
    variant_label: str
    stacker_components: list[str]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run persistence-heavy stacking experiments for Stage 05.")
    ...
    parser.add_argument(
        "--skip-holdout",
        action="store_true",
        help="Run validation sweep only and do not open holdout for the top-two new stack candidates.",
    )
    return parser
...
def build_specs() -> list[StackSpec]:
    return [
        StackSpec(
            stage_id="05BU",
            run_name="05BU_frontier_stack_w_bf_0001",
            experiment_id="exp_05bu_frontier_stack_w_bf_v1",
            review_basename="05BU_mt5_model_family_trial_review",
            variant_label="stack_05w_bf",
            stacker_components=["base_05w", "persistence_volatility"],
        ),
        StackSpec(
            stage_id="05BV",
            run_name="05BV_frontier_stack_w_bb_bf_0001",
            experiment_id="exp_05bv_frontier_stack_w_bb_bf_v1",
            review_basename="05BV_mt5_model_family_trial_review",
            variant_label="stack_05w_bb_bf",
            stacker_components=["base_05w", "persistence_only", "persistence_volatility"],
        ),
        StackSpec(
            stage_id="05BW",
            run_name="05BW_frontier_stack_w_bb_ah_0001",
            experiment_id="exp_05bw_frontier_stack_w_bb_ah_v1",
            review_basename="05BW_mt5_model_family_trial_review",
            variant_label="stack_05w_bb_fullproxy",
            stacker_components=["base_05w", "persistence_only", "full_proxy"],
        ),
    ]


def run_trial(spec: StackSpec, args: argparse.Namespace, *, run_holdout: bool) -> None:
    cmd = [
        sys.executable,
...
def extract_payload(view: BundleView, spec: StackSpec | None, label: str) -> dict[str, Any]:
    extra = view.execution.get("extra") or {}
    return {
        "run_name": view.run_name,
        "variant_label": label,
        "stacker_components": [] if spec is None else spec.stacker_components,
...
def build_review_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "phase": "05BY_mt5_persistence_frontier_stack_batch",
    ...
    if ranked_holdout is not None:
        ...
        if holdout_winner not in REFERENCE_RUN_NAMES and validation_winner == holdout_winner:
            payload["verdict"] = {
                ...
                "reason": "persistence-heavy stacking blend led both validation and holdout across the current frontier",
            }
        elif holdout_winner not in REFERENCE_RUN_NAMES:
            payload["verdict"] = {
                "status": "mixed_holdout_win_keep_incumbent",
                "selected_run_name": incumbent_name,
                "reason": "stacking blend improved holdout but did not lead validation, so the incumbent stays promoted for now",
            }
        else:
            payload["verdict"] = {
...
def build_review_markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["ranked_validation_runs"]:
        components = ", ".join(row["stacker_components"]) if row["stacker_components"] else "-"
        lines.append(
            f"- [{row['rank']}] `{row['run_name']}`: "
            f"variant=`{row['variant_label']}`, components=`{components}`, "
...
def update_review_index(review_dir: Path) -> None:
    ...
    entry = "- `05BY`: see `05BY_mt5_persistence_frontier_stack_review.md`"
...
def main() -> int:
    ...
    review_json_path = review_dir / "05BY_mt5_persistence_frontier_stack_review.json"
    review_md_path = review_dir / "05BY_mt5_persistence_frontier_stack_review.md"
