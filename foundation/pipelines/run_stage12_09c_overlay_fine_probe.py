...
STAGE12_ROOT = ROOT_DIR / "stages" / "12_09c_overlay_fine_probe"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a tiny 3x3 fine probe around the Stage 11 Monday/post-cash overlay pocket on top of the 09C core."
    )
    parser.add_argument("--stage-root", default=str(STAGE12_ROOT), help="Stage 12 root directory.")
    parser.add_argument("--source-root", default=str(STAGE09_ROOT), help="Stage 09 active runs root.")
    parser.add_argument("--batch-id", default="12OF", help="Stage 12 batch id.")
    ...
    parser.add_argument(
        "--review-basename",
        default="12OF_09c_overlay_fine_probe_review",
...
def stage12_paths(stage_root: Path) -> dict[str, Path]:
    return {
...
def build_variants() -> list[OverlayVariant]:
    monday_values = [0.60, 0.625, 0.65]
    post_values = [0.60, 0.625, 0.65]
    stage_ids = ["12A", "12B", "12C", "12D", "12E", "12F", "12G", "12H", "12I"]
    variants: list[OverlayVariant] = []
    index = 0
    for monday_mult in monday_values:
        for post_mult in post_values:
            monday_token = str(monday_mult).replace(".", "")
            post_token = str(post_mult).replace(".", "")
            label = f"monday_{monday_mult:.3f}_post_{post_mult:.3f}".replace(".000", "").replace(".625", "0625")
            variants.append(
                OverlayVariant(
                    stage_id=stage_ids[index],
                    token=f"M{monday_token}_P{post_token}",
                    label=label,
                    description=f"Monday={monday_mult:.3f}, post-cash={post_mult:.3f}",
                    runtime_extra={
...
def ensure_stage_scaffold(stage_root: Path, paths: dict[str, Path], args: argparse.Namespace, variants: list[OverlayVariant]) -> None:
    ...
    manifest = {
        ...
        "reference_overlay_stage11": ["11B monday_065_post065", "11D monday_065_post060"],
        ...
        write_text(
            ...
            "\n".join(
                [
                    ...
                    "- status: `running`",
                    "- source core: `09C_05ET_margin0700_t30_l50_m0700_h4`",
                    "- objective: `see whether a tiny 3x3 overlay probe can improve 2407 holdout_b beyond Stage 11`",
                    "",
                ]
            ),
        )
...
def update_selection_status(selection_path: Path, leader: dict[str, Any], source_reference: dict[str, Any]) -> None:
    lines = [
        ...
        "- next action: `decide whether this Stage 12 fine-probe overlay leader is strong enough to replace the Stage 11 winner`",
        "",
    ]
...
def main() -> int:
    ...
    paths = stage12_paths(stage_root)
    ...
    review_payload = {
        "generated_at_utc": utc_now_iso(),
        "stage": "12_09c_overlay_fine_probe",
        "phase": "12OF_09c_overlay_fine_probe",
