...
DEFAULT_MODEL_CONFIG_PATH = ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05DP_05ca_margin0675_hold5_0001" / "config.json"
DEFAULT_LOGIC_RULE_STACK_PATH = ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05ER_05dp_short_bias_margin_hold4_0001" / "rule_stack.json"
DEFAULT_STANDARD_REFERENCE_ROOT = STAGE06_ROOT / "02_runs" / "active"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Retrain the LP01/LP02 source model on an earlier WFO window and compare the two session overlays."
    )
    ...
    parser.add_argument(
        ...
        help="Source rule_stack.json used as the base logic for the WFO probe.",
    )
    parser.add_argument("--batch-id", default="06F", help="Stage 06 batch id.")
    ...
    parser.add_argument(
        "--review-basename",
        default="06F_early_wfo_lp_compare_review",
...
def overlay_variants(batch_id: str) -> list[dict[str, Any]]:
    return [
        {
            "token": "LP01",
            "run_name": f"{batch_id}_WFO_LP01",
            "experiment_id": f"exp_{batch_id.lower()}_early_wfo_lp01_v1",
            "label": "local_probe_a",
            "description": "Softer Monday, tighter post-cash decay.",
            "standard_reference_run": "06E_R250_LP01",
            "runtime_extra": {
        ...
        {
            "token": "LP02",
            "run_name": f"{batch_id}_WFO_LP02",
            "experiment_id": f"exp_{batch_id.lower()}_early_wfo_lp02_v1",
            "label": "local_probe_b",
            "description": "Stronger Monday and post-cash decay.",
            "standard_reference_run": "06E_R250_LP02",
            "runtime_extra": {
...
def split_dataset(
    ...
    base = dataset[valid_mask].copy()
    base["timestamp"] = pd.to_datetime(base["timestamp"], utc=True)
    return {
        "train": base[(base["timestamp"] >= train_start) & (base["timestamp"] < train_end)].copy(),
        "validation": base[(base["timestamp"] >= train_end) & (base["timestamp"] < validation_end)].copy(),
        "test": base[(base["timestamp"] >= validation_end) & (base["timestamp"] < test_end)].copy(),
    }


def evaluate_split(model: object, split_df: pd.DataFrame, active_features: list[str]) -> tuple[dict[str, Any], pd.DataFrame]:
    x_split = split_df[active_features]
    ...
    out = split_df[["timestamp", "forward_return", "label"]].copy()
...
def patch_bundle(
    *,
    ...
    bundle.data_snapshot.extra.update(wfo_metadata)
    bundle.runtime_snapshot.extra.update(runtime_extra)
...
def build_run_payload(
    *,
    ...
    split_boundaries: SplitBoundaries,
    segment_scheme: dict[str, list[tuple[str, datetime, datetime]]],
    ...
    return {
        ...
        "phase": "06F_early_wfo_lp_compare",
...
def build_batch_review_markdown(
    *,
...
def update_selection_status(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "- stage: `06_segmented_risk_validation`",
        "- current phase complete: `06F`",
        "- standard-window leader remains: `06E_R250_LP01`",
        f"- early WFO leader: `{leader['run_name']}` `{leader['label']}`",
        ...
        f"- early WFO holdout headline: `return_pct={format_metric(holdout.get('return_pct'), 3)}`, `PF={format_metric(holdout.get('profit_factor'), 4)}`, `trades={holdout.get('trade_count')}`, `max_dd_pct={format_metric(holdout.get('max_dd_pct'), 4)}`",
        "- next action: `decide whether the earlier-window WFO winner is strong enough to reuse on nearby risk levels or adjacent walk-forward windows`",
    ]
...
def main() -> int:
    ...
    variants = overlay_variants(args.batch_id)
    for variant in variants:
        ...
        config_payload = {
            **base_config,
            "run_name": variant["run_name"],
            "phase": "stage06_early_wfo_lp_compare",
            "generated_at_utc": utc_now_iso(),
        ...
        bundle = patch_bundle(
            ...
            wfo_metadata={
                ...
                "wfo_phase": "06F_early_wfo_lp_compare",
                "overlay_label": variant["label"],
            },
        )
        ...
        write_text(run_dir / "segmented_results.md", build_batch_review_markdown(payloads=[payload], split_boundaries=split_boundaries))
    ...
    write_text(review_md_path, build_batch_review_markdown(payloads=review_payloads, split_boundaries=split_boundaries))
    ...
    update_selection_status(stage_paths["selection"], leader=leader, split_boundaries=split_boundaries)
    write_json(
        stage_root / "01_inputs" / "06F_early_wfo_manifest.json",
        {
            "generated_at_utc": utc_now_iso(),
            "phase": "06F_early_wfo_lp_compare",
