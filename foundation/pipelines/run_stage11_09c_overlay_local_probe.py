...
STAGE11_ROOT = ROOT_DIR / "stages" / "11_09c_overlay_local_probe"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a very narrow local multiplier probe around the Stage 10 LP02 overlay on top of the 09C core."
    )
    parser.add_argument("--stage-root", default=str(STAGE11_ROOT), help="Stage 11 root directory.")
    parser.add_argument("--source-root", default=str(STAGE09_ROOT), help="Stage 09 active runs root.")
    parser.add_argument("--batch-id", default="11OL", help="Stage 11 batch id.")
    ...
    parser.add_argument(
        "--review-basename",
        default="11OL_09c_overlay_local_probe_review",
...
def stage11_paths(stage_root: Path) -> dict[str, Path]:
    return {
...
def build_variants() -> list[OverlayVariant]:
    return [
        OverlayVariant(
            stage_id="11A",
            token="LP02",
            label="lp02_reference",
            description="Current Stage 10 LP02 reference.",
            runtime_extra={
                "monday_risk_pct_mult": 0.70,
                "ny_postcash_risk_pct_mult": 0.65,
                "session_overlay_label": "lp02_reference",
            },
        ),
        OverlayVariant(
            stage_id="11B",
            token="MON065",
            label="monday_065_post065",
            description="Stronger Monday decay only.",
            runtime_extra={
                "monday_risk_pct_mult": 0.65,
                "ny_postcash_risk_pct_mult": 0.65,
                "session_overlay_label": "monday_065_post065",
            },
        ),
        OverlayVariant(
            stage_id="11C",
            token="POST060",
            label="monday_070_post060",
            description="Stronger post-cash decay only.",
            runtime_extra={
                "monday_risk_pct_mult": 0.70,
                "ny_postcash_risk_pct_mult": 0.60,
                "session_overlay_label": "monday_070_post060",
            },
        ),
        OverlayVariant(
            stage_id="11D",
            token="BOTH0605",
            label="monday_065_post060",
            description="Stronger Monday and post-cash decay together.",
            runtime_extra={
                "monday_risk_pct_mult": 0.65,
                "ny_postcash_risk_pct_mult": 0.60,
                "session_overlay_label": "monday_065_post060",
            },
        ),
        OverlayVariant(
            stage_id="11E",
            token="MON075",
            label="monday_075_post065",
            description="Softer Monday with current post-cash decay.",
            runtime_extra={
                "monday_risk_pct_mult": 0.75,
                "ny_postcash_risk_pct_mult": 0.65,
                "session_overlay_label": "monday_075_post065",
            },
        ),
        OverlayVariant(
            stage_id="11F",
            token="POST070",
            label="monday_070_post070",
            description="Current Monday with softer post-cash decay.",
            runtime_extra={
                "monday_risk_pct_mult": 0.70,
                "ny_postcash_risk_pct_mult": 0.70,
                "session_overlay_label": "monday_070_post070",
            },
        ),
    ]
...
def ensure_stage_scaffold(stage_root: Path, paths: dict[str, Path], args: argparse.Namespace, variants: list[OverlayVariant]) -> None:
        ...
        write_text(
            ...
            "\n".join(
                [
                    "# Stage Brief",
                    "",
                    "- stage: `11_09c_overlay_local_probe`",
                    "- goal: `run a very narrow multiplier probe around the winning Stage 10 LP02 overlay on top of the 09C core`",
                    "- source core: `09C_05ET_margin0700_t30_l50_m0700_h4`",
                    "- windows: `2401, 2407, 2501`",
                    "- target: `2407 / holdout_b`",
                    "- local axes: `monday_risk_pct_mult, ny_postcash_risk_pct_mult`",
                    "",
                ]
            ),
        )
    ...
    manifest = {
        ...
        "reference_overlay": "10C local_probe_b",
        ...
        write_text(
            ...
            "\n".join(
                [
                    ...
                    "- status: `running`",
                    "- source core: `09C_05ET_margin0700_t30_l50_m0700_h4`",
                    "- objective: `see whether a tiny multiplier adjustment around LP02 can improve 2407 holdout_b beyond Stage 10`",
                    "",
                ]
            ),
        )
...
def update_selection_status(selection_path: Path, leader: dict[str, Any], source_reference: dict[str, Any]) -> None:
    lines = [
        ...
        "- next action: `decide whether this Stage 11 local overlay winner is strong enough to replace the Stage 10 overlay leader`",
        "",
    ]
...
def main() -> int:
    ...
    paths = stage11_paths(stage_root)
    ...
    review_payload = {
        "generated_at_utc": utc_now_iso(),
        "stage": "11_09c_overlay_local_probe",
        "phase": "11OL_09c_overlay_local_probe",
