...
STAGE08_ROOT = ROOT_DIR / "stages" / "08_strict_wfo_shortlist"
...
class CandidateSpec:
    ...
    base_name: str
    label: str
    core_reference_run_name: str
    standard_reference_run_name: str
    rule_stack_path: Path
    motif_source_run_name: str | None = None
    motif_label: str | None = None
    motif_temperature: float = 1.0
    motif_class_biases: tuple[float, float, float] = (0.0, 0.0, 0.0)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a narrow Stage 08 strict-retrain WFO shortlist for 05ER/05EM/05ET and the 07E-style 05FD motif transplant."
    )
    parser.add_argument("--stage-root", default=str(STAGE08_ROOT), help="Stage 08 root directory.")
    ...
    parser.add_argument("--batch-id", default="08SW", help="Stage 08 batch id.")
    parser.add_argument("--random-state", type=int, default=42, help="Random seed for retraining.")
    parser.add_argument("--risk-pct", type=float, default=2.0, help="Risk percent for the shortlist runs.")
    ...
    parser.add_argument(
        "--review-basename",
        default="08SW_strict_wfo_shortlist_review",
...
def stage08_paths(stage_root: Path) -> dict[str, Path]:
    return {
...
def build_candidates() -> list[CandidateSpec]:
    return [
        CandidateSpec(
            stage_id="08A",
            base_name="05er_core",
            label="05ER_core_short_bias_hold4",
            core_reference_run_name="05ER_05dp_short_bias_margin_hold4_0001",
            standard_reference_run_name="05ER_05dp_short_bias_margin_hold4_0001",
            rule_stack_path=ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05ER_05dp_short_bias_margin_hold4_0001" / "rule_stack.json",
        ),
        CandidateSpec(
            stage_id="08B",
            base_name="05em_core",
            label="05EM_core_short_bias_hold5",
            core_reference_run_name="05EM_05dp_short_bias_margin_hold5_0001",
            standard_reference_run_name="05EM_05dp_short_bias_margin_hold5_0001",
            rule_stack_path=ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05EM_05dp_short_bias_margin_hold5_0001" / "rule_stack.json",
        ),
        CandidateSpec(
            stage_id="08C",
            base_name="05et_core",
            label="05ET_core_long_suppression_hold4",
            core_reference_run_name="05ET_05dp_stronger_long_suppression_hold4_0001",
            standard_reference_run_name="05ET_05dp_stronger_long_suppression_hold4_0001",
            rule_stack_path=ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05ET_05dp_stronger_long_suppression_hold4_0001" / "rule_stack.json",
        ),
        CandidateSpec(
            stage_id="08D",
            base_name="07e_fd_motif",
            label="05ET_plus_05FD_motif",
            core_reference_run_name="05ET_05dp_stronger_long_suppression_hold4_0001",
            standard_reference_run_name="07E_05et_fd_motif_0001",
            rule_stack_path=ROOT_DIR / "stages" / "05_optimization" / "02_runs" / "active" / "05ET_05dp_stronger_long_suppression_hold4_0001" / "rule_stack.json",
            motif_source_run_name="05FD_05ca_flatup_longdown_margin0675_hold5_0001",
            motif_label="flat_up_long_down_bias",
            motif_temperature=1.0,
            motif_class_biases=(0.0, 0.1, -0.1),
        ),
    ]
        ...
        write_text(
            ...
            "\n".join(
                [
                    "# Stage Brief",
                    "",
                    "- stage: `08_strict_wfo_shortlist`",
                    "- goal: `run a narrow strict-retrain WFO shortlist for 05ER/05EM/05ET core candidates plus the 07E motif transplant`",
                    "- windows: `2401, 2407, 2501`",
                    "- ranking focus: `worst holdout_b defense first, then average test return`",
                    "- intent: `separate robust core choice from motif uplift and avoid another broad search pass`",
                    "",
                ]
            ),
        )
        ...
        "candidates": [
            {
                ...
                "rule_stack_path": str(spec.rule_stack_path.resolve()),
                "motif_source_run_name": spec.motif_source_run_name,
                "motif_label": spec.motif_label,
                "motif_temperature": spec.motif_temperature,
                "motif_class_biases": list(spec.motif_class_biases),
            }
        ...
        write_text(
            ...
            "\n".join(
                [
                    ...
                    "- stage: `08_strict_wfo_shortlist`",
                    "- status: `running`",
                    "- shortlist: `05ER, 05EM, 05ET, 07E-style 05FD motif`",
                    "- next action: `wait for the strict-retrain shortlist review before promoting any core or motif variant`",
                    "",
                ]
            ),
        )
...
def window_run_name(spec: CandidateSpec, window_token: str) -> str:
...
def apply_output_shape_motif(model: Any, spec: CandidateSpec) -> Any:
    if spec.motif_source_run_name is None:
        return copy.deepcopy(model)
    transformed = copy.deepcopy(model)
    classifier = transformed.named_steps["classifier"]
    scale_vector = np.asarray((1.0, 1.0, 1.0), dtype=np.float64) / float(spec.motif_temperature)
    bias_vector = np.asarray(spec.motif_class_biases, dtype=np.float64)
    ...
    payload.update(
        {
            ...
            "phase": "stage08_strict_wfo_shortlist",
            "generated_at_utc": utc_now_iso(),
            ...
            if spec.motif_source_run_name is None
                ...
                "motif_source_run_name": spec.motif_source_run_name,
                "motif_label": spec.motif_label,
                "temperature": spec.motif_temperature,
                "class_biases": [float(value) for value in spec.motif_class_biases],
            },
        }
    )
    return payload
...
def build_run_payload(
    *,
    ...
    return {
        "generated_at_utc": utc_now_iso(),
        "stage": "08_strict_wfo_shortlist",
        "phase": "08SW_strict_wfo_shortlist",
        "run_name": window_run_name(spec, window_token),
        ...
        if spec.motif_source_run_name is None
            ...
            "source_run_name": spec.motif_source_run_name,
            "label": spec.motif_label,
            "temperature": spec.motif_temperature,
            "class_biases": [float(value) for value in spec.motif_class_biases],
        },
...
def segment_metric(payload: dict[str, Any], split_name: str, segment_name: str, metric_name: str) -> float:
    ...
    if isinstance(segments, dict):
        ...
        segment = next(
            ...
            if str(item.get("segment") or "") == segment_name
        )
    return float(segment["headline"].get(metric_name) or 0.0)


def aggregate_candidates(payloads: list[dict[str, Any]], candidates: list[CandidateSpec]) -> list[dict[str, Any]]:
    ...
    for stage_id, runs in by_candidate.items():
        ...
        holdout_b_pfs = [segment_metric(run, "test", "holdout_b", "profit_factor") for run in runs]
        validation_returns = [float(run["split_runs"]["validation"]["headline"].get("return_pct") or 0.0) for run in runs]
        worst_run = min(runs, key=lambda run: segment_metric(run, "test", "holdout_b", "return_pct"))
        rows.append(
            {
                ...
                "avg_validation_return_pct": float(np.mean(validation_returns)),
                ...
                "avg_holdout_b_profit_factor": float(np.mean(holdout_b_pfs)),
                ...
                "worst_window_token": worst_run["window_token"],
                "worst_window_holdout_b_return_pct": segment_metric(worst_run, "test", "holdout_b", "return_pct"),
                "per_window": [
                    {
...
def build_review_markdown(aggregated_rows: list[dict[str, Any]], payloads: list[dict[str, Any]]) -> str:
    lines = [
    ...
    for row in aggregated_rows:
        motif_label = row["motif_source_run_name"] if row["motif_source_run_name"] else "none"
        lines.append(
            f"- [{row['rank']}] `{row['candidate_stage_id']}` `{row['candidate_label']}`: "
            f"core=`{row['core_reference_run_name']}`, motif=`{motif_label}`, "
            f"avg_test=`{format_metric(row['avg_test_return_pct'], 3)}`, min_test=`{format_metric(row['min_test_return_pct'], 3)}`, "
            f"avg_holdout_b=`{format_metric(row['avg_holdout_b_return_pct'], 3)}`, min_holdout_b=`{format_metric(row['min_holdout_b_return_pct'], 3)}`, "
            f"positive_test_windows=`{row['positive_test_windows']}/{row['window_count']}`, positive_holdout_b_windows=`{row['positive_holdout_b_windows']}/{row['window_count']}`, "
            f"worst_window=`{row['worst_window_token']}`"
        )
    ...
    leader = aggregated_rows[0] if aggregated_rows else None
    if leader is not None:
        lines.extend(
            [
                "## Readout",
                "",
                f"- shortlist leader: `{leader['candidate_stage_id']}` `{leader['candidate_label']}`",
                f"- leader min_holdout_b: `{format_metric(leader['min_holdout_b_return_pct'], 3)}`",
                f"- leader avg_test_return: `{format_metric(leader['avg_test_return_pct'], 3)}`",
...
def update_selection_status(selection_path: Path, leader: dict[str, Any]) -> None:
    lines = [
        ...
        "- status: `completed`",
        f"- shortlist leader: `{leader['candidate_stage_id']}` `{leader['candidate_label']}`",
        f"- leader basis: `min_holdout_b={format_metric(leader['min_holdout_b_return_pct'], 3)}, avg_test_return={format_metric(leader['avg_test_return_pct'], 3)}, min_test_return={format_metric(leader['min_test_return_pct'], 3)}`",
        f"- leader lineage: `core={leader['core_reference_run_name']}`, `motif={leader['motif_source_run_name'] or 'none'}`",
        "- next action: `use this shortlist result to decide whether the robust core wins on its own or the 07E-style motif is worth a final narrow probe`",
        "",
    ]
...
def export_bundle_for_run(
    *,
    ...
    export_args = argparse.Namespace(
        ...
        stage_name="strict_wfo_shortlist",
...
def main() -> int:
    ...
    paths = stage08_paths(stage_root)
    ...
    for window_token, train_start_raw, train_end_raw, validation_end_raw, test_end_raw in WINDOW_SPECS:
        ...
        for spec in candidates:
            ...
            candidate_model = apply_output_shape_motif(base_model, spec)
            validation_metrics, validation_predictions = evaluate_split(candidate_model, validation_df, active_features)
            test_metrics, test_predictions = evaluate_split(candidate_model, test_df, active_features)
            offline_metrics = {"validation": validation_metrics, "test": test_metrics}

            joblib.dump(candidate_model, run_dir / "model.joblib")
            ...
            rule_stack = load_json(spec.rule_stack_path)
            ...
            write_json(
                run_dir / "candidate_manifest.json",
                {
                    ...
                    "motif_label": spec.motif_label,
                    "motif_temperature": spec.motif_temperature,
                    "motif_class_biases": [float(value) for value in spec.motif_class_biases],
                },
            )
            write_json(
                run_dir / "config.json",
            ...
            if not bundle_path.exists() or args.rebuild_bundle:
                bundle_path = export_bundle_for_run(
                    ...
                    spec=spec,
                    args=args,
                    max_hold_bars=int(rule_stack["exit"][0]["params"]["max_hold_bars"]),
                )
            ...
            patch_bundle(
                ...
                wfo_metadata={
                    ...
                    "wfo_phase": "08SW_strict_wfo_shortlist",
                    "candidate_stage_id": spec.stage_id,
                    "candidate_label": spec.label,
                },
            )
    ...
    aggregated_rows = aggregate_candidates(payloads, candidates)
    ...
    review_payload = {
        ...
        "aggregated_candidates": aggregated_rows,
        "runs": payloads,
    }
    write_json(review_json_path, review_payload)
    write_text(review_md_path, build_review_markdown(aggregated_rows, payloads))
    update_review_index(paths["review_index"], review_md_path.name)

    if aggregated_rows:
        update_selection_status(paths["selection"], aggregated_rows[0])

    print(f"[done] review={review_md_path}")
    for row in aggregated_rows:
        print(
            f"[done] candidate={row['candidate_stage_id']} avg_test={row['avg_test_return_pct']:.3f} "
            f"min_holdout_b={row['min_holdout_b_return_pct']:.3f}"
        )
    return 0
